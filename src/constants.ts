...
export const createRegularExpression = (
  catalog: string,
  flags: string = "gi"
) => {
  let output = `${catalog}`;
...
export const regularExpressionfilter = (
  data: string[],
  regExps: RegExpFilterData["includes"]
) => {
  return data.filter((path) => {
    if (regExps.filter((r) => toRoot(path).match(r) !== null).length === 0)
      return false;
...
export const log = (action: string, name: string) => {
  console.log(action + " \u001B[33;1m" + name + "\u001B[0m" + "...");
};
